...
func (_q *DiscordUserQuery) QueryMessages() *DiscordMessageQuery {
...
func (_q *DiscordUserQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *DiscordUserQuery) FirstIDX(ctx context.Context) string {
...
func (_q *DiscordUserQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *DiscordUserQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *DiscordUserQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *DiscordUserQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *DiscordUserQuery) WithMessages(opts ...func(*DiscordMessageQuery)) *DiscordUserQuery {
	query := (&DiscordMessageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(discorduser.FieldUsername).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.DiscordUser.Query().
//		Select(discorduser.FieldUsername).
//		Scan(ctx, &v)
...
func (_q *DiscordUserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*DiscordUser, error) {
	...
	if query := _q.withMessages; query != nil {
		if err := _q.loadMessages(ctx, query, nodes,
			...
			func(n *DiscordUser, e *DiscordMessage) { n.Edges.Messages = append(n.Edges.Messages, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DiscordUserQuery) loadMessages(ctx context.Context, query *DiscordMessageQuery, nodes []*DiscordUser, init func(*DiscordUser), assign func(*DiscordUser, *DiscordMessage)) error {
	...
	query.Where(predicate.DiscordMessage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(discorduser.MessagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.discord_message_author
		if fk == nil {
			return fmt.Errorf(`foreign-key "discord_message_author" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "discord_message_author" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *DiscordUserQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(discorduser.Table, discorduser.Columns, sqlgraph.NewFieldSpec(discorduser.FieldID, field.TypeString))
	_spec.From = _q.sql

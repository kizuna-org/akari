...
func TestAkariUserRepository_GetOrCreateAkariUserByDiscordUserID(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			userID: "user-002",
			setupMock: func(m *interactorMock.MockAkariUserInteractor) {
				m.EXPECT().GetAkariUserByDiscordUserID(gomock.Any(), "user-002").Return(nil, errors.New("not found"))
		...
		},
		{
			...
			userID: "user-003",
			setupMock: func(m *interactorMock.MockAkariUserInteractor) {
				m.EXPECT().GetAkariUserByDiscordUserID(gomock.Any(), "user-003").Return(nil, errors.New("not found"))

...
> 人格ごとの調整パラメータ**とします（→ [12.7](./12-persona.md)）。
> 既定は人間に近く狭いですが、広く設定した人格（多くを並行して意識できる）も
> 仕様として認めます。ただし**広さは疲れに直結する**ため
> （疲れは意識に上がった量で溜まる → [07. 7.8](./07-autonomy.md)）、
> 広げるだけでは「何でも並行して考えられる人格」にはなりません。
...
**Channel は固定の5個ではありません。** 人間準拠で考えると、こうなります
（→ [01. 1.3](./01-vision.md)）。

| | 常設か | 数 |
|---|---|---|
| **器官にあたるもの**（会話・実行・情動評価・記憶想起） | 常に動いている | 型としてひとつずつ |
| **いま考えている筋道**（着想・個々の作業） | 対象があるときだけ | **対象ごとに立ち上がり、済めば消える** |

`hu.` 口はひとつだが、頭の中で並行して考えていることの数は決まっていない

- 調べもの・作業・気がかりは、**それぞれが独立した思考の系列**として立ち上がります。
  同時に三つのことを気にかけているなら、そこには三つの系列があります。
- 済んだ系列、関心が尽きた系列は消えます（→ [やることpool](#67-やることpool未処理の行動候補を束ねる場)の `dropped`）。
- **いくつまで並行できるか**は無制限ではなく、人格パラメータで決まります
  （→ [12.7](./12-persona.md) の考えごとの多さ）。

> **将来**：どう分けるかの判断自体も、いずれ主体に委ねます。
> いまは型を決めておくほうが確実ですが、人間は「これは別の系列で考えよう」と
> 外から決められてはいません（→ [01. 1.3](./01-vision.md)）。
...
- **どれくらい粘るかは人格パラメータ**（→ [12.7](./12-persona.md) のやることpool の粘り）。
  いつまでも気にしている人格も、すぐ「もういいや」となる人格も作れます。
  固定の保持時間は設けません（時間で切るのは人間らしくないため）。
...
**意識層の広さ・切り替えやすさ・慣れの速さ・考えごとの多さ**は、
[12. 人格パラメータ](./12-persona.md) の調整軸として外に出しました。

あわせて **やることpool の粘り** も人格パラメータになり、固定の保持時間は設けません（6.7）。
**Channel 構成**は「型は固定・数は動的」に決めました（6.3）。
**慣れ**（6.6）は人間準拠なので仕様には入れます（導入の時期は実装側の判断）。

本書に残るのは次の一点です。

1. **並列の同時性の見せ方**：複数の思考の系列が同時に動いていることを、ユーザーに
   どこまで見せますか（「考え中」「作業中」の可視化の有無）。
   これは仕様というより見せ方の話なので、実体の段階（→ [10](./10-roadmap.md)）と
   合わせて決めるのが自然です。

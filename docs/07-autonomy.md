...
2. **自発行動の積極性**：静かに見守り気味 ↔ どんどん動く、のどのあたりを初期値にしますか。
   [7.8](#78-自発行動の抑制自制心とシステムリミット) の疲れ・遠慮の強さの初期値がこれにあたります。
3. **意図の割り込みやすさ**：いま抱えている意図を、どれくらいの強さの割り込みで
   中断してよいか（→ [05. 目標と意図](./05-goal.md) の「やり通す強さ」と対応）。
4. **疲れは何で溜まるか**：行為の回数で溜まるのか、考えた量（意識に上がった回数）でも
   溜まるのか。後者なら「考えすぎて疲れる」が表現できますが、複雑になります。
5. **欲求・動機の明示化**：休息欲は疲れとして入りましたが、好奇心・承認欲求などを

...
- **無限ループ防止**：最大反復数を設ける（**足場**。本来は「もう駄目だ」と自分で
  切り上げるべきもので、諦めの判断が自前でできるようになったら外す
  → [01. 1.3](./01-vision.md)）。
- **目標達成判定**を明確にする。
...
> **関門は錠であって足場ではありません**（→ [01. 1.3](./01-vision.md)）。
> LLM が賢くなっても外しません。人間の社会にも鍵と法があるのと同じです。
> ここも[原則10](./01-vision.md)の二段構えで、**通常は自分の判断で手が止まり**、
> 関門はその外側に立ちます。

### 最低応答の保証（錠）

自律的に動く存在は、「気が乗らないので返さない」が起こりえます。
人間もそうなので、**振る舞いとしては正しい**のですが、
相手にとっては壊れているのと区別がつきません。

ここも二段構えにします（→ [01. 原則10](./01-vision.md)）。

| 段 | 何が働くか |
|---|---|
| **内側：良識** | 呼ばれたら返す。返せないなら「あとで」と言う。これが通常の動き |
| **外側：錠** | 自分に向けられた呼びかけには、**一定時間内に必ず何かを返す** |

- **返す中身は問いません。** 「いま手が離せない」「あとで見る」でも構いません。
  錠が保証するのは**沈黙しないこと**だけです。
- 錠が発動するのは異常です。日常的に発動するなら、
  遠慮や疲れの設定が強すぎるということです（→ [7.8](#78-自発行動の抑制自制心とシステムリミット)）。
- **この下限は人格で動かせません**（→ [12.10](./12-persona.md)）。
  「そういう人格だから無視する」は認めません。

`hu.` 手が離せなくても「ちょっと待って」とは言う
...
> これは [01. 原則10](./01-vision.md)（止める仕組みは理性と錠の二段構え）の一例です。
> 本書ではこの形をいちばん詳しく書いていますが、**同じ形が機密・安全・最低応答にも
> 当てはまります**。
...
- **最低応答の保証を錠として置く**（→ [7.7](#77-安全と境界ハード制約)）。呼びかけに対し
  一定時間内に必ず何かを返す。中身は問わない。この下限は人格で動かせない。
- **この二段構えは自発行動に限らない**。機密・安全・最低応答も同じ形で扱う
  （→ [01. 原則10](./01-vision.md)）。
...
- **最低応答保証** → **設けます**。二段構えの錠として [7.7](#77-安全と境界ハード制約) に
  追加しました。下限は人格で動かせません。
- **欲求・動機の明示化** → **これ以上増やしません**。休息欲は疲れ、好奇心は関心として
  すでに形があります。承認欲求などをさらに作り込むのは足場を増やすことであり、
  「なぜ LLM に任せられないか」を書けません（→ [01. 1.3](./01-vision.md)）。
  人が誰かに認められたいと思う程度のことは、人格と記憶と関心があれば出てきます。
